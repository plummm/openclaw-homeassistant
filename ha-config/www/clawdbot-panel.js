...
(function(){
  ...
  async function renderAgentView(){
    ...
    try{
      ...
      if (img) {
        img.src = `/api/clawdbot/agent_avatar.png?ts=${Date.now()}`;
  ...
  function bindAvatarUi(){
    ...
    const openCropper = (file) => {
      ...
      img.onload = () => {
        ...
        modal.innerHTML = `
          <div class="modal-card" style="max-width:520px">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:10px">
              <div style="font-weight:900">Crop avatar</div>
              <button class="btn" id="avClose">Close</button>
            </div>
            <div class="muted" style="margin-top:6px">Drag to position. Use zoom slider. Output is 512×512 PNG.</div>
            <div style="margin-top:12px;display:flex;justify-content:center">
              <canvas id="avCanvas" width="${SIZE}" height="${SIZE}" style="width:${Math.min(360,SIZE)}px;height:${Math.min(360,SIZE)}px;border-radius:18px;border:1px solid var(--cb-border-strong);background:var(--ha-card-background,var(--card-background-color));touch-action:none"></canvas>
            </div>
            <div style="margin-top:12px;display:flex;gap:10px;align-items:center">
              <div class="muted" style="min-width:44px">Zoom</div>
              <input id="avZoom" type="range" min="1" max="4" step="0.01" value="1" style="flex:1" />
            </div>
            <div style="margin-top:14px;display:flex;justify-content:flex-end;gap:10px">
              <button class="btn" id="avReset">Reset</button>
              <button class="btn primary" id="avSave">Save</button>
            </div>
          </div>`;
        document.body.appendChild(modal);

        const canvas = modal.querySelector('#avCanvas');
        const ctx = canvas.getContext('2d');
        const zoom = modal.querySelector('#avZoom');

        const clamp = () => {
          const z = parseFloat(zoom.value);
          const w = img.width * minScale * z;
          const h = img.height * minScale * z;
          const minX = (SIZE - w);
          const minY = (SIZE - h);
          if (w <= SIZE) ox = 0; else ox = Math.max(minX/2, Math.min(-minX/2, ox));
          if (h <= SIZE) oy = 0; else oy = Math.max(minY/2, Math.min(-minY/2, oy));
        };
        ...
        let drag = false;
        let sx = 0, sy = 0;
        canvas.addEventListener('pointerdown', (ev)=>{ drag=true; sx=ev.clientX; sy=ev.clientY; canvas.setPointerCapture(ev.pointerId); });
        canvas.addEventListener('pointermove', (ev)=>{
          ...
          const k = SIZE / Math.max(1, rect.width);
          ox += dx * k;
          oy += dy * k;
          clamp();
          draw();
        ...
        zoom.addEventListener('input', ()=>{ clamp(); draw(); });
      ...
      img.onerror = () => { toast('Failed to load image'); try{ URL.revokeObjectURL(url); }catch(e){} };
      img.src = url;
    };
